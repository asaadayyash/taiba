...
<div class="flex items-center justify-between mb-8">
  <div>
    <h1 class="text-3xl font-bold text-[#F5F0E8] playfair">Categories</h1>
    <p class="text-[#666] mt-1"><?= count($categories) ?> categories</p>
  </div>
...
<div class="card-dark rounded-2xl p-6 mb-8">
  <h2 class="font-bold text-[#F5F0E8] mb-5">Add Category</h2>
  <form method="post" class="max-w-md space-y-4">
    <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
    <input type="hidden" name="action" value="create">
    <div>
      <label class="block text-[#A0998A] text-sm mb-1.5">Name *</label>
    ...
    <div>
      <label class="block text-[#A0998A] text-sm mb-1.5">Slug *</label>
    ...
    <div class="flex gap-3">
      <a href="/taiba-store/admin/categories.php" class="px-6 py-2.5 rounded-xl border border-[#2A2A2A] text-[#666] text-sm">Cancel</a>
...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
  ...
  <div class="card-dark rounded-2xl p-5 flex items-center gap-4">
    <div class="w-12 h-12 bg-[#C9A840]/10 rounded-xl flex items-center justify-center text-2xl flex-shrink-0">🏷</div>
    <div class="flex-1 min-w-0">
      <p class="font-semibold text-[#F5F0E8] text-sm"><?= e($cat['name']) ?></p>
      <p class="text-xs text-[#555] mt-0.5">/<?= e($cat['slug']) ?></p>
      <p class="text-xs text-[#666] mt-1"><?= $cat['product_count'] ?> products</p>
    </div>
    <form method="post" onsubmit="return confirm('Delete <?= e($cat['name']) ?>?')">
      <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
      <input type="hidden" name="action" value="delete">
      <input type="hidden" name="id" value="<?= $cat['id'] ?>">
      <button type="submit" class="text-[#555] hover:text-red-400 transition-colors">🗑</button>
    </form>
  </div>
  <?php endforeach; ?>
</div>
