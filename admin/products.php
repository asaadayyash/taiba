...
<div class="flex items-center justify-between mb-8">
  <div>
    <h1 class="text-3xl font-bold text-[#F5F0E8] playfair">Products</h1>
    <p class="text-[#666] mt-1"><?= count($products) ?> total</p>
  </div>
...
<div class="card-dark rounded-2xl p-6 mb-8">
  <h2 class="font-bold text-[#F5F0E8] mb-5"><?= $editP ? 'Edit Product' : 'Add Product' ?></h2>
  <form method="post">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="md:col-span-2">
        <label class="block text-[#A0998A] text-sm mb-1.5">Product Name *</label>
      ...
      <div class="md:col-span-2">
        <label class="block text-[#A0998A] text-sm mb-1.5">Description</label>
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-1.5">Price (SAR) *</label>
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-1.5">Compare Price</label>
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-1.5">Category *</label>
      ...
      <div class="md:col-span-2">
        <label class="block text-[#A0998A] text-sm mb-1.5">Image URLs <span class="text-[#555]">(one per line)</span></label>
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2 text-sm text-[#A0998A] cursor-pointer">
          <input type="checkbox" name="featured" value="1" <?= !empty($editP['featured']) ? 'checked' : '' ?> class="accent-[#C9A840]"/> Featured
        </label>
        <label class="flex items-center gap-2 text-sm text-[#A0998A] cursor-pointer">
          <input type="checkbox" name="active" value="1" <?= ($editP['active'] ?? 1) ? 'checked' : '' ?> class="accent-[#C9A840]"/> Active
        </label>
      </div>
    </div>

    <div class="flex gap-3 mt-6">
      <a href="/taiba-store/admin/products.php" class="px-6 py-2.5 rounded-xl border border-[#2A2A2A] text-[#666] text-sm hover:text-[#A0998A]">Cancel</a>
    ...
    <table class="w-full">
      ...
      <tbody>
        ...
        <tr class="border-b border-[#1A1A1A] hover:bg-[#161616]">
          <td class="px-6 py-4">
            <p class="font-medium text-[#F5F0E8] text-sm"><?= e($p['name']) ?></p>
            <?php if (!empty($p['material'])): ?><p class="text-xs text-[#555]"><?= e($p['material']) ?></p><?php endif; ?>
          </td>
          <td class="px-6 py-4 text-sm text-[#A0998A]"><?= e($p['cat_name']) ?></td>
          <td class="px-6 py-4 text-sm font-semibold text-[#C9A840]"><?= formatPrice($p['price']) ?></td>
          <td class="px-6 py-4 text-sm <?= $p['stock']>0?'text-green-400':'text-red-400' ?>"><?= $p['stock'] ?></td>
          <td class="px-6 py-4">
            <div class="flex flex-col gap-1">
              <?php if ($p['featured']): ?><span class="text-xs bg-[#C9A840]/20 text-[#C9A840] px-2 py-0.5 rounded-full w-fit">Featured</span><?php endif; ?>
          ...
          <td class="px-6 py-4">
            <div class="flex items-center gap-3">
              <a href="?edit=<?= $p['id'] ?>" class="text-[#666] hover:text-[#C9A840]">✏️</a>
              <form method="post" onsubmit="return confirm('Delete this product?')">
                <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                <input type="hidden" name="action" value="delete">
                <input type="hidden" name="id" value="<?= $p['id'] ?>">
                <button type="submit" class="text-[#666] hover:text-red-400">🗑</button>
              </form>
            </div>
          </td>
        </tr>
        ...
        <tr><td colspan="6" class="px-6 py-12 text-center text-[#555]">No products yet.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
