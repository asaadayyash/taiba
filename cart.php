...
<div class="py-8 pb-20 max-w-6xl mx-auto px-6">
  <h1 class="text-3xl font-bold text-[#F5F0E8] mb-8 playfair">Shopping Cart</h1>
  ...
  <div class="text-center py-20">
    ...
    <p class="text-[#666] text-lg mb-6">Your cart is empty</p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-4">
      ...
      <div class="card-dark rounded-2xl p-4 flex gap-4">
        <a href="/taiba-store/product.php?id=<?= $item['product_id'] ?>">
          <div class="w-24 h-24 rounded-xl overflow-hidden bg-[#1E1E1E] flex-shrink-0">
            <?php if (!empty($imgs[0])): ?>
              <img src="<?= e($imgs[0]) ?>" alt="<?= e($item['name']) ?>" class="w-full h-full object-cover"/>
            <?php else: ?>
              <div class="w-full h-full flex items-center justify-center text-[#333] text-2xl playfair">T</div>
            <?php endif; ?>
          </div>
        </a>
        <div class="flex-1 min-w-0">
          <p class="text-xs text-[#C9A840] mb-1"><?= e($item['cat_name']) ?></p>
          <a href="/taiba-store/product.php?id=<?= $item['product_id'] ?>"><p class="font-medium text-[#F5F0E8] text-sm mb-2"><?= e($item['name']) ?></p></a>
          <p class="text-[#C9A840] font-bold"><?= formatPrice($item['price']) ?></p>
        </div>
        <div class="flex flex-col items-end justify-between">
          ...
          <form method="post" action="/taiba-store/api/cart.php">
            ...
            <button type="submit" class="text-[#555] hover:text-red-400 transition-colors text-sm">🗑</button>
          </form>
          <!-- Quantity -->
          <form method="post" action="/taiba-store/api/cart.php" class="flex items-center gap-2 bg-[#1E1E1E] rounded-full px-3 py-1.5">
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="action" value="update">
            <input type="hidden" name="product_id" value="<?= $item['product_id'] ?>">
            <input type="hidden" name="redirect" value="/taiba-store/cart.php">
            <button name="quantity" value="<?= $item['quantity'] - 1 ?>" class="text-[#666] hover:text-[#C9A840] w-5 text-center">−</button>
            <span class="text-[#F5F0E8] text-sm w-5 text-center"><?= $item['quantity'] ?></span>
            <button name="quantity" value="<?= $item['quantity'] + 1 ?>" class="text-[#666] hover:text-[#C9A840] w-5 text-center">+</button>
          </form>
        </div>
      </div>
    ...
    <div>
      <div class="card-dark rounded-2xl p-6 sticky top-24">
        <h2 class="font-bold text-[#F5F0E8] mb-5 text-lg">Order Summary</h2>
        <div class="space-y-3 text-sm mb-5">
          <div class="flex justify-between"><span class="text-[#666]">Subtotal</span><span><?= formatPrice($subtotal) ?></span></div>
          <div class="flex justify-between"><span class="text-[#666]">VAT (15%)</span><span><?= formatPrice($tax) ?></span></div>
          <div class="flex justify-between"><span class="text-[#666]">Shipping</span><span class="<?= $shipping===0?'text-green-400':'' ?>"><?= $shipping===0?'Free':formatPrice($shipping) ?></span></div>
          <div class="border-t border-[#2A2A2A] pt-3 flex justify-between font-bold text-base">
            <span>Total</span><span class="text-[#C9A840]"><?= formatPrice($total) ?></span>
          </div>
        </div>
        ...
        <form method="post" action="/taiba-store/api/order.php" id="checkoutForm">
          ...
          <div id="checkoutFields" class="hidden space-y-3 mb-4">
            <h3 class="font-semibold text-[#F5F0E8] text-sm">Delivery Details</h3>
            ...
            <button type="button" onclick="toggleCheckout()" class="w-full py-2 text-sm text-[#666] hover:text-[#A0998A]">← Back to Cart</button>
          </div>
