...
<div class="card-dark card-hover rounded-2xl overflow-hidden group">
  <a href="/taiba-store/product.php?id=<?= (int)$p['id'] ?>">
    <div class="relative aspect-square bg-[#dbc894] overflow-hidden">
      <?php if ($imgSrc): ?>
      ...
      <?php else: ?>
        <div class="w-full h-full flex items-center justify-center">
          <span class="text-[#c4b49a] text-6xl playfair">T</span>
        </div>
      <?php endif; ?>
      <?php if ($onSale): ?>
  ...
  <div class="p-4">
    <p class="text-xs text-[#8d9d4f] mb-1"><?= e($p['cat_name'] ?? '') ?></p>
    <a href="/taiba-store/product.php?id=<?= (int)$p['id'] ?>">
      <h3 class="font-medium text-[#5c4b3e] hover:text-[#8d9d4f] transition-colors line-clamp-2 text-sm leading-snug mb-2"><?= e($p['name']) ?></h3>
    </a>
    <?php if (!empty($p['material'])): ?>
      <p class="text-xs text-[#85766a] mb-3"><?= e($p['material']) ?></p>
    <?php endif; ?>
    <div class="flex items-center justify-between mt-2">
      <div>
        <span class="font-bold text-[#8d9d4f]"><?= formatPrice($p['price']) ?></span>
        <?php if ($onSale): ?>
          <span class="text-xs text-[#a08878] line-through ml-2"><?= formatPrice($p['compare_price']) ?></span>
        <?php endif; ?>
      </div>
      ...
      <?php else: ?>
        <span class="bg-[#b19681] text-[#a08878] text-xs px-3 py-1.5 rounded-full">Out</span>
      <?php endif; ?>
    </div>
  </div>
</div>
