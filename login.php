...
<div class="min-h-screen flex items-center justify-center px-4 py-12 relative">
  ...
  <div class="w-full max-w-md">
    <div class="text-center mb-8">
      ...
      <h1 class="text-2xl font-bold text-[#5c4b3e] mt-6 mb-2">Welcome Back</h1>
      <p class="text-[#85766a] text-sm">Sign in to your account</p>
    </div>
    ...
    <form method="post" class="card-dark rounded-2xl p-8 space-y-5">
      ...
      <div>
        <label class="block text-[#85766a] text-sm mb-2" for="email">Email Address</label>
      ...
      <div>
        <label class="block text-[#85766a] text-sm mb-2" for="password">Password</label>
        <div class="relative">
          ...
          <button type="button" onclick="const f=this.previousElementSibling;f.type=f.type==='password'?'text':'password';this.textContent=f.type==='password'?'👁':'🙈'" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#85766a] text-sm">👁</button>
        </div>
      </div>
      ...
      <p class="text-center text-[#85766a] text-sm">
        Don't have an account? <a href="/taiba-store/signup.php" class="text-[#8d9d4f] hover:underline">Create one</a>
      </p>
    </form>
  </div>
</div>
