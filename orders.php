...
<div class="py-8 pb-20 max-w-4xl mx-auto px-6">
  <h1 class="text-3xl font-bold text-[#F5F0E8] mb-8 playfair">My Orders</h1>
  ...
  <div class="text-center py-20">
    ...
    <p class="text-[#666] text-lg">No orders yet</p>
      ...
      <div class="p-5 flex flex-wrap items-start justify-between gap-3 cursor-pointer" onclick="this.nextElementSibling.classList.toggle('hidden')">
        <div>
          <div class="flex items-center gap-3 flex-wrap">
            <span class="font-mono text-[#C9A840] font-bold text-sm"><?= e($order['order_number']) ?></span>
            <?= statusBadge($order['status']) ?>
          </div>
          <p class="text-[#666] text-xs mt-1"><?= date('d M Y', strtotime($order['created_at'])) ?> · <?= $order['item_count'] ?> items</p>
        </div>
        <span class="font-bold text-[#F5F0E8]"><?= formatPrice($order['total']) ?></span>
      </div>
      <div class="hidden border-t border-[#2A2A2A] p-5 space-y-2">
        ...
        <?php endforeach; ?>
        <div class="pt-2 border-t border-[#1A1A1A] text-xs text-[#555]">
