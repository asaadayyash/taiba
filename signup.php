...
<div class="min-h-screen flex items-center justify-center px-4 py-12 relative">
  ...
  <div class="w-full max-w-md">
    <div class="text-center mb-8">
      ...
      <h1 class="text-2xl font-bold text-[#F5F0E8] mt-6 mb-2">Create Account</h1>
      <p class="text-[#666] text-sm">Join our luxury jewelry family</p>
    </div>
    ...
    <form method="post" class="card-dark rounded-2xl p-8 space-y-5">
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-2">Full Name</label>
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-2">Email Address</label>
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-2">Phone <span class="text-[#555]">(optional)</span></label>
      ...
      <div>
        <label class="block text-[#A0998A] text-sm mb-2">Password</label>
        <div class="relative">
          ...
          <button type="button" onclick="const f=document.getElementById('pw');f.type=f.type==='password'?'text':'password'" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#666] text-sm">👁</button>
        </div>
        <div id="strength" class="mt-2 hidden">
          <div class="flex gap-1">
            <div id="s1" class="h-1 flex-1 rounded-full bg-[#2A2A2A]"></div>
            <div id="s2" class="h-1 flex-1 rounded-full bg-[#2A2A2A]"></div>
            <div id="s3" class="h-1 flex-1 rounded-full bg-[#2A2A2A]"></div>
          </div>
        </div>
      </div>
      <div>
        <label class="block text-[#A0998A] text-sm mb-2">Confirm Password</label>
      ...
      <p class="text-center text-[#666] text-sm">
        Already have an account? <a href="/taiba-store/login.php" class="text-[#C9A840] hover:underline">Sign in</a>
      </p>
    </form>
  </div>
</div>
...
function updateStrength(pw) {
  ...
  bars.forEach((b,i) => b.className = 'h-1 flex-1 rounded-full ' + (i < score ? colors[score-1] : 'bg-[#2A2A2A]'));
}
</script>
